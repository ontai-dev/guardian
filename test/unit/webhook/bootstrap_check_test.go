...
const testOperatorNamespace = "seam-system"

func operatorNamespace(labels map[string]string) *corev1.Namespace {
...
// The operator namespace carries seam.ontai.dev/webhook-mode=exempt — the
...
// Operator namespace has no labels — `compiler enable` has not been run.
...
func TestCheckBootstrapLabels_LabelAbsent_ReturnsErrBootstrapLabelAbsent(t *testing.T) {
	s := bootstrapCheckScheme(t)
	ns := operatorNamespace(nil) // no labels
	c := fake.NewClientBuilder().WithScheme(s).WithObjects(ns).Build()
...
// If the operator namespace itself is absent, something is fundamentally wrong.
...
func TestCheckBootstrapLabels_NamespaceNotFound_ReturnsError(t *testing.T) {
	s := bootstrapCheckScheme(t)
	// No operator namespace in the fake client.
	c := fake.NewClientBuilder().WithScheme(s).Build()

	err := webhook.CheckBootstrapLabels(context.Background(), c, testOperatorNamespace)
	if err == nil {
		t.Error("expected non-nil error when operator namespace absent; got nil")
	}
	if errors.Is(err, webhook.ErrBootstrapLabelAbsent) {
