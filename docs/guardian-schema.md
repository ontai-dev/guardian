...
`kubeconfigSecretRef` is not in the CR and no kubeconfig Secret is pre-generated or
manually applied. Platform derives the tenant cluster kubeconfig on the fly from the
talosconfig Secret (`seam-mc-{clusterName}-talosconfig` in seam-system) after the
InfrastructureTalosCluster CR is admitted.
...
**seam-tenant-{clusterName}-namespace.yaml** -- the seam-tenant namespace manifest:

The namespace manifest for `seam-tenant-{clusterName}` is generated by the compiler
bootstrap subcommand for import mode and is part of the pre-admission artifact set.

**Apply order (Governor action, before CR admission):**
The Governor applies these three artifacts to the management cluster in the following
order before the InfrastructureTalosCluster CR is admitted:

1. `seam-tenant-{clusterName}-namespace.yaml` -- creates the tenant namespace
2. `seam-mc-{clusterName}-talosconfig.yaml` -- stores the talosconfig Secret
3. Any additional machineconfig secrets captured from cluster-input.yaml

The InfrastructureTalosCluster CR is applied last, after all prerequisites are in place.
Platform reads the talosconfig Secret from seam-system on first reconcile and derives
the tenant cluster kubeconfig on the fly using the Talos API. No kubeconfig Secret is
pre-generated or manually applied by the Governor.
...
two-site orchestration. For the management cluster side, the seam-tenant-{clusterName}
namespace and the talosconfig Secret are already present (applied by the Governor as
part of the pre-admission artifact set in Step 1). Platform reads the talosconfig Secret
(`seam-mc-{clusterName}-talosconfig` in seam-system) on first reconcile and derives the
tenant cluster kubeconfig on the fly via the Talos API. No kubeconfig Secret is
pre-generated or stored by any actor outside Platform.

**Management cluster side (seam-tenant-{clusterName}):**
1. Verify `seam-tenant-{clusterName}` namespace exists (created by Governor from compiler
   output before CR admission -- see Step 1 apply order).
2. Create InfrastructureTalosCluster ownerReference on the tenant namespace (CP-INV-008).

**Tenant cluster side (executed via tenant kubeconfig derived from talosconfig):**
3. Create namespace `ont-system`.
4. Create conductor ServiceAccount in ont-system.
5. Create conductor Deployment (role=tenant) in ont-system.
...
6. Create the conductor RBACProfile in ont-system (see Step 4) -- gated on conductor
   Deployment being Ready (see Step 5, item 6 ordering dependency).
...
6. Guardian admission webhook becomes functional on the tenant cluster for Seam-governed
   ...
   **Ordering dependency (Point 3):** The conductor Deployment in ont-system must be
   fully Running and its webhook server endpoint registered with the tenant cluster API
   server before the RBACProfile in Step 4 can be admitted. If Platform attempts to
   create the RBACProfile before the conductor webhook is Ready, the admission request
   will be rejected because there is no webhook handler to accept it. Platform must gate
   RBACProfile creation on a readiness check of the conductor Deployment (`status.readyReplicas
   >= 1`). This is an ordering dependency that Platform is solely responsible for enforcing.
...
**RBAC precondition for handshake completion write (Point 2 -- implementation gap):**
The status write in the handshake completion step requires management conductor to have
`update` and `patch` verbs on the `infrastructuretalosclusters/status` subresource in
`infrastructure.ontai.dev`. Confirmed absent: the current `conductor-permissions`
PermissionSet (generated by the compiler) does not include this rule. The `management-maximum`
ceiling is wildcard and covers it, but the actual ClusterRole provisioned by Guardian for
the conductor ServiceAccount is generated from `conductor-permissions` -- the specific rule
is missing there.

This is an implementation gap that blocks Step 5 handshake completion. The following rule
must be added to `conductor-permissions` in the compiler (`writeBootstrapPermissionSets` in
`compile_enable.go`) and the enable bundle regenerated before Step 5 can function end-to-end:
...
This gap is tracked under COMPILER-BL-PERMISSIONSET-DEFECT and will be addressed in the
compiler fix session that also removes the per-operator PermissionSet defect.
